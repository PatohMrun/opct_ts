...
const Navbar = () => {
    ...
    const handleOutsideClick = (e: MouseEvent) => {
        if (isOpen && menuRef.current && !menuRef.current.contains((e.target as Node) || null)) {
            setIsOpen(false);
        }
    };
    ...
    return (
        <nav className="font-poppins bg-primary flex justify-between items-center gap-8 h-12 relative w-full z-30">
            ...
            <div className="mx-2 sm:mx-4 flex items-center">
                <Link href="/"><Image src={"/images/OPCT.png"} alt="OPCT Logo" width={30} height={30} /></Link>
                <Link href="/" className="text-xs text-white mx-2">OPCT</Link>
            </div>
            ...
            <div className="flex items-center">
                ...
                <div className="sm:hidden ml-2 mr-2">
                    {isOpen ? (
                        <IoClose className="text-secondary text-3xl" onClick={handleToggleMenu} />
                    ) : (
                        <IoMenu className="text-secondary text-3xl" onClick={handleToggleMenu} />
                    )}
                </div>
            </div>
